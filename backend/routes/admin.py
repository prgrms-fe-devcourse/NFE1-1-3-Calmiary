...
.modal-content pre {
    ...
    <body>
       <div class="container">
        ...
        <div class="controls">
            <button class="button refresh-btn" onclick="refreshData()">새로고침</button>
            <button class="button reset-btn" onclick="resetDatabase()">DB 초기화</button>
        </div>
        ...
        <div class="tab-container">
            <div class="tab-buttons">
                <button class="tab-button active" onclick="showTab('users')">사용자</button>
            ...
            <div id="users" class="tab-content active">
                <button class="add-button" onclick="showModal('userModal')">사용자 추가</button>
                <table id="usersTable">
                    <thead>
                        <tr>
                            <th>ID</th>
    ...
    <button class="add-button" onclick="showModal('postModal')">게시글 추가</button>
    <table id="postsTable">
        <thead>
            <tr>
                <th>ID</th>
        ...
        <tbody></tbody>
            ...
            <div id="comments" class="tab-content">
                <button class="add-button" onclick="showModal('commentModal')">댓글 추가</button>
            ...
            <div id="likes" class="tab-content">
                <button class="add-button" onclick="showModal('likeModal')">좋아요 추가</button>
                <table id="likesTable">
                    <thead>
                        <tr>
                            <th>ID</th>
    ...
    <div id="likeModal" class="modal">
        ...
        // 사용자 목록 업데이트 (post와 comment 모달용)
        if (modalId === 'postModal' || modalId === 'commentModal') {
            updateUserSelect(modal.querySelector('select[name="user_id"]'));
        }
        
        // 게시글 목록 업데이트 (comment 모달용)
        if (modalId === 'commentModal') {
            updatePostSelect(modal.querySelector('select[name="post_id"]'));
        }
    }
    ...
    document.getElementById('commentForm').onsubmit = async function(e) {
        ...
        try {
            const response = await fetch(`/community/post/${data.post_id}/comment`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
...
function showPostDetail(postId) {
                ...
                <div class="modal-content" style="width: 90%; max-width: 800px;">
                    ...
                    <div style="margin-top: 20px;">
                        ...
                        <div style="margin-top: 20px;">
                            ...
                            <p><strong>상태:</strong> ${post.is_solved ? '해결됨' : '미해결'} / ${post.is_shared ? '공개' : '비공개'}</p>
                        </div>
                    </div>
                </div>
            `;
    ...
    async function toggleLike(postId, userId) {
        try {
            const response = await fetch(`/community/post/${postId}/like?user_id=${userId}`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
